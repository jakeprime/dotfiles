...
(add-to-list 'custom-theme-load-path "~/.config/spacemacs/")
  ...
  (defvar omarchy/omarchy-current-theme-file
  (concat (getenv "XDG_CONFIG_HOME") "/omarchy/current/theme.name"))

(defun omarchy/--read-current-theme-name ()
  (f-read-text omarchy/omarchy-current-theme-file))

(defvar omarchy/loaded-theme (omarchy/--read-current-theme-name))


(defun jake/--theme-watch-callback (_event)
  (message "CHANGED!")
  (message (concat omarchy/loaded-theme " / " (omarchy/--read-current-theme-name)))
  (add-to-list 'custom-theme-load-path "~/.config/spacemacs/")
  (load-theme 'jakewave t))

(defun jake/--add-theme-watch ()
  ;; (setq omarchy/loaded-theme (omarchy/--current-theme))
  (file-notify-add-watch omarchy/omarchy-current-theme-file
                         '(change)
                         #'jake/--theme-watch-callback))

(jake/--add-theme-watch)
...
(defun jake-smerge-fix-markers (start end)
  "Strip all syntax highlighting from smerge conflict marker lines."
  (save-excursion
    (goto-char start)
    (while (re-search-forward
      ...
      (put-text-property (match-beginning 0) (match-end 0)
                         'font-lock-face 'smerge-markers))))

(add-hook 'smerge-mode-hook
          (lambda ()
            (if smerge-mode
                (add-hook 'jit-lock-functions #'jake-smerge-fix-markers t t)
              (remove-hook 'jit-lock-functions #'jake-smerge-fix-markers t))
            (font-lock-flush)))
