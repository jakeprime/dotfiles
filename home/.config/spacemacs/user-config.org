...
** General
*** Lexical binding
As of Emacs 31 Lexical binding has to be declared, as either =t= or =nil=, depending on if using the newer Lisp dialect. This must be the first entry.

#+begin_src emacs-lisp :tangle "layer-config.el"
;; -*- lexical-binding: nil -*-
#+end_src

#+begin_src emacs-lisp :tangle "user-env.el"
;; -*- lexical-binding: nil -*-
#+end_src
...
**** LSP
I'm using treesitter with =ruby-ts-mode= and this doesn't hook into the LSP by default. =ruby-base-mode= is the parent of both =ruby-mode= and =ruby-ts-mode=.
...
**** Sorbet
The =sig= blocks have normal ruby syntax highlighting which is very distracting, move them into the background with its own subtle styling.
...
*** YAML
Enable code-folding in YAML pages.
...
** Documents
Update evil navigation to work on visual lines in visual mode, used in Markdown and Org mode.
...
** Email
To set up email:
1. Install =isync= and =mu=, with brew or pacman
   Cleo will require OAuth2. Howebrew
2. Create mailbox folders, e.g. =mkdir -p ~/.local/share/mail/personal=
3. Sync with =mbsync -a=
4. Set up =mu= with
   =mu init --maildir==~/.local/share/mail --my-address=jake@jakeprime.com --my-address=jake.prime@gmail.com --my-address=jake@meetcleo.com=
   =mu index=
...
*** Cleo OAuth2 setup
Cleo requires OAuth2, which requires a few steps:

- Install =pip3 install --user mutt-oauth2= to be able to authorise and get a token
- Initialize with =mutt-oauth2 --authorize --username jake@meetcleo.com=
- Build =mbsync= from source to support it. Following instructions for Mac, maybe don't bother on Arch:

#+begin_src sh
  brew install autoconf automake libtool pkg-config cyrus-sasl openssl

  curl -LO https://downloads.sourceforge.net/project/isync/isync/1.5.1/isync-1.5.1.tar.gz
  tar xf isync-1.5.1.tar.gz
  cd isync-1.5.1
  PKG_CONFIG_PATH="/opt/homebrew/opt/cyrus-sasl/lib/pkgconfig:/opt/homebrew/opt/openssl@3/lib/pkgconfig" \
    LDFLAGS="-L/opt/homebrew/opt/cyrus-sasl/lib -L/opt/homebrew/opt/openssl@3/lib" \
    CPPFLAGS="-I/opt/homebrew/opt/cyrus-sasl/include -I/opt/homebrew/opt/openssl@3/include" \
  ./configure --prefix="$HOME/.local" --with-sasl=/opt/homebrew/opt/cyrus-sasl --with-ssl=/opt/homebrew/opt/openssl@3
  make
  make install
  ...
  make
  make install
  PKG_CONFIG_PATH="/opt/homebrew/opt/cyrus-sasl/lib/pkgconfig" ./configure --prefix="$HOME/.local" --with-cyrus-sasl=/opt/homebrew/opt/cyrus-sasl
#+end_src
...
*** Conflicts
The Ruby syntax highlighting takes precedence over the smerge font faces when displaying a merge conflict, so we have to do this:
