...
Sorbet's LSP appears to return symbol information in a different format to ruby-ls. Locations are nested and they don't have a =location= key. Because we are running both LSPs at once all refs are processed in one list and the Sorbet ones throw an error. This patch filters out symbols where a  =location= key doesn't exist.
